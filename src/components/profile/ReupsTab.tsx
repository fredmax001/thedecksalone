  ...
  if (!reups || reups.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-lg font-medium">No re-ups yet</p>
        <p className="text-sm mt-2">Pro DJs can re-share mixes they love from other DJs.</p>
      </motion.div>
    );
  }
    ...
    if (mix.audioUrl) {
      window.dispatchEvent(
        new CustomEvent('play-mix', {
          detail: { track: mix, queue: reups.map((r) => r.mix).filter((m) => m.audioUrl) },
        })
      );
    }
  };
          ...
          <div className="p-4">
            ...
            {item.mix.dj && (
              <p className="mt-1 text-xs text-gold truncate">
                Original by {item.mix.dj.stageName}
